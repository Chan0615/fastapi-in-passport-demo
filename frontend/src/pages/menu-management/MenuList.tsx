...
function sortMenus(list: MenuInfo[]) {
  return [...list].sort((a, b) => {
    ...
    if (getTypeOrder(a.menu_type) !== getTypeOrder(b.menu_type)) {
      return getTypeOrder(a.menu_type) - getTypeOrder(b.menu_type);
    }
    return a.id - b.id;
  });
}
...
function buildMenuTree(list: MenuInfo[]): MenuTreeNode[] {
  ...
  const trim = (nodes: MenuTreeNode[]): MenuTreeNode[] =>
    nodes.map((n) => ({
      ...n,
      children: n.children && n.children.length ? trim(n.children) : undefined,
    }));
...
export default function MenuList() {
  ...
  const stat = useMemo(() => {
    const directory = menus.filter((m) => m.menu_type === 'directory').length;
    const menu = menus.filter((m) => m.menu_type === 'menu').length;
    const button = menus.filter((m) => m.menu_type === 'button').length;
  ...
  const getExcludedIds = () => {
    ...
    menus.forEach((m) => {
      ...
      children.push(m.id);
      childMap.set(m.parent_id, children);
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'is_visible',
      width: 80,
      render: (value: boolean) => (value ? <Tag color="green">显示</Tag> : <Tag>隐藏</Tag>),
    },
    {
      title: '排序',
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Text type="secondary">目录 {stat.directory}</Text>
          <Text type="secondary">菜单 {stat.menu}</Text>
          <Text type="secondary">按钮 {stat.button}</Text>
